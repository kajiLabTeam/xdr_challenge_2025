...
test_trials = {
    "test": {  # a trial used by the test suite
        "name": "test",
...
try:
    ...
    Swagger(
        app, config=swagger_config, template=swagger_template, sanitizer=MK_SANITIZER
    )
except ImportError:
    pass
...
class Triallog:
    ...
    def log_response(self, trial, response):
        self.__log(
            "%s --> %d, trialts=%.3f rem=%.3f"
            % (
                self.__date(),
                response.status_code,
                trial["trialts"],
                remaining_time(trial),
            )
        )
        ...
        for name in initrials:
            ...
            if name in trials:
                ...
                if state != "nonstarted":
                    if debug:
                        print(
                            f"Trial {name} not reloaded from init file {globinifn} because it is {state}"
                        )
                    continue
            ## Create trial
            trials[name] = {
            ...
            try:
                for k in (  # selected keys from initrial
                    ...
                    trials[name][k] = initrial[k]
                    # copy from initrial
                if debug:
                    print(f"Trial {name}: {trials[name]}")
                ...
                if not trials[name]["offline"]:
                    assert (
                        isinstance(initrial["V"], (int, float)) and initrial["V"] >= 1
                    )
                else:
                    assert (
                        isinstance(initrial["V"], (int, float)) and initrial["V"] == 0
                    )
            ...
            if debug:
                print(f"Trial {name} loaded from {conforigin}")
    ...
    if op == "complete":
        ...
        for trialname in trials:
            if trials[trialname]["state"] == "running":
                ts[trialname] = {}
                ts[trialname]["ts"] = trials[trialname]["trialts"]
                ts[trialname]["remaining"] = remaining_time(trials[trialname])
...
def trial_reload(trialname):
    ...
    if debug:
        print(f"File {logfn} did{'' if logfnexisted else ' not'} exist")
    ...
    if debug:
        print(f"File {logfn} does{'' if logfnexists else ' not'} exist")
    ...
    if not trial["reloadable"]:
        if logfnexisted:
            ...
            if debug:
                print(f"Reloading non-reloadable trial {trialname}")
...
def trial_state(trialname, trial=None, dolog=True):
    ...
    if state == "nonstarted":
        ...
        if trial["offline"]:
            ...
            rem = -1
        data = statefmt.format(
            trialts=0, rem=rem, V=V, S=S, p=0, h=0, pts=0, pos=inipos
        )
        if debug:
            print(f"State is {state}: {data}")
...
def trial_nextdata(trialname):
    ...
    if not trial["offline"]:  # online trial
        ...
        if state in ("running", "finished"):
            ...
            if "horizon" in request.args:
                ...
                try:
                    ...
                    r = Response(
                        f'Horizon value not a number: "{horizon}" -- request ignored',
                        422,
                    )
                    log.log_response(trial, r)
                    return r
                if h < 0 or h == float("inf"):  # negative or infinite
                    r = Response(
                        f'Horizon negative or inf: "{horizon}" -- request ignored', 422
                    )
                    log.log_response(trial, r)
                    return r
            else:
            ...
            if (
                trialts > trial["inits"]  # position at 0 is the initial position
        ...
        if state in ("finished", "timeout"):
            ...
            log.log_response(trial, r)
            log.log_msg(
                f"Trial finished {'normally' if state == 'finished' else 'by timeout'}\n"
            )
            close_trial(trial)
            return r
    ...
    while (
        trial["offline"] or linets < trialts + h
    ):  # while line in horizon or offline trial
...
def post_trial_estimates(trialname):
    ...
    if not rejreason:
        ...
        r = Response(
            accmsg + f", {rejno} rejected, first at line {rejfirst}: {rejreason}", 409
        )
    log.log_response(trial, r)
...
def trial_log(trialname):
    ...
    if "log" in trial:  # log file is still open
        if debug:
            print(f"Reopening log file {logfn}")
        trial["log"].reopen()  # flush the compressed data
