    ...
    if hasattr(localizer, "get_raw_measurements"):
        ...
        for tag_id, measurements in raw_measurements.items():
            if measurements:
                ...
                raw_csv_file = (
                    output_dir / f"{trial_id}_{datetime}_tag_{tag_id}_raw.csv"
                )
                ...
                logger.info(
                    f"  - Total: {total_count}, LOS: {los_count}, NLOS: {nlos_count} ({nlos_ratio:.1f}%)"
                )
    ...
    if hasattr(localizer, "get_tag_trajectories_with_los"):
        ...
        for tag_id, trajectory in tag_trajectories_with_los.items():
            if trajectory:
                ...
                logger.info(
                    f"Tag {tag_id} の推定軌跡を {tag_csv_file} に保存しました (重み付き平均)"
                )
