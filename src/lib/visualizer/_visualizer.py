        ...
        for idx, (tag_id, trajectory) in enumerate(trajectories_to_plot.items()):
            ...
            if use_los_info:
                    ...
                    tag_df = pd.DataFrame(
                        [
                            (
                                ...
                                pos.confidence,
                                pos.method,
                                getattr(pos, "gpos_distance", 0.0),
                                getattr(pos, "is_far_from_gpos", False),
                            )
                ...
                if "gpos_distance" in tag_df.columns:
                    print(
                        f"Distance range: {tag_df['gpos_distance'].min():.2f} - {tag_df['gpos_distance'].max():.2f}m"
                    )
                ...
                if blue_only:
                    ...
                    # 統計情報を更新（青色のみの場合）
                    filtered_ratio = (
                        len(blue_points) / len(tag_df) * 100 if len(tag_df) > 0 else 0
                    )

                    # タイトルに情報追加
                    ax.set_title(
                        f"Tag {tag_id} - Reliable Points Only\\n"
                    ...
                    if not close_to_gpos_df.empty:
                        ...
                        nlos_close = close_to_gpos_df[
                            close_to_gpos_df["is_los"] == False
                        ]
                        ...
                        if not los_close.empty:
                            ax.scatter(
                                ...
                                edgecolors="darkblue",
                                linewidth=0.5,
                                label=f"LOS & <3m from GPOS ({len(los_close)} points)",
                                zorder=3,
                            )
                        ...
                        if not nlos_close.empty:
                            ax.scatter(
                                ...
                                linewidth=0.5,
                                marker="s",  # 四角でNLOSを表示
                                label=f"NLOS & <3m from GPOS ({len(nlos_close)} points)",
                                zorder=3,
                            )
                    ...
                    if not far_from_gpos_df.empty:
                        ...
                        if not los_far.empty:
                            ax.scatter(
                                ...
                                label=f"LOS & ≥3m from GPOS ({len(los_far)} points)",
                                zorder=4,
                            )
                        ...
                        if not nlos_far.empty:
                            ax.scatter(
                                ...
                                label=f"NLOS & ≥3m from GPOS ({len(nlos_far)} points)",
                                zorder=5,
                            )
                ...
                info_text += (
                    f"Distance traveled: {self._calculate_total_distance(tag_df):.2f} m"
                )
            ...
            if use_los_info:
                has_nlos = (
                    any(not pos.is_los for pos in trajectory)
                    if hasattr(trajectory[0], "is_los")
                    else False
                )
                nlos_count = (
                    sum(1 for pos in trajectory if not pos.is_los)
                    if hasattr(trajectory[0], "is_los")
                    else 0
                )
                ...
                if has_nlos:
                    ax.set_title(
                        f"UWB Position Estimation - Tag {tag_id} (LOS: {los_count}, NLOS: {nlos_count})"
                    )
                else:
                    ax.set_title(
                        f"UWB Position Estimation - Tag {tag_id} (LOS Only: {los_count} points)"
                    )
            else:
                ax.set_title(
                    f"UWB Position Estimation - Tag {tag_id} ({len(trajectory)} points)"
                )
            ...
            if save:
                ...
                tag_output_file = (
                    output_path.parent
                    / f"{output_path.stem}_tag_{tag_id}{output_path.suffix}"
                )
                self.logger.info(
                    f"Tag {tag_id} のプロットを {tag_output_file} に保存します"
                )
