...
class PDRLocalizer(DataRecorderProtocol):
    ...
    def _detect_device_orientation(self) -> bool:
        ...
        if len(self.gyro_datarecorder.data) < 100:
            self.logger.warning("ジャイロデータが不足しています。デフォルト設定を使用します")
            return True  # 001.txtベースのデフォルト
        
        # 分析用データを取得（約20秒分のデータを使用）
        gyro_fs = self.gyro_datarecorder.fs  # サンプリング周波数
        target_duration = 20.0  # 20秒
        analysis_samples = min(int(gyro_fs * target_duration), len(self.gyro_datarecorder.data))
        ...
        for i in range(n_sections):
            ...
            end_idx = (i + 1) * section_size if i < n_sections - 1 else len(gyr_x_values)
        ...
        if confidence_threshold < 30:
            # 回転が少ない場合：デフォルト設定を使用
            self.logger.info(f"回転量が少ないため({cumulative_rotation_deg:.1f}°)、デフォルト設定を使用")
            return True  # 001.txtベースのデフォルト
