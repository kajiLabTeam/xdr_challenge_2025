...
class UWBLocalizer(DataRecorderProtocol):
    ...
    def save_blue_only_trajectories_to_csv(
        ...
        try:
            ...
            blue_trajectories = self._uwb_create_blue_only_trajectories()
    ...
    def get_blue_only_measurements(self) -> Dict[str, List[PositionWithLOS]]:
        ...
        for tag_id, measurements in self.raw_measurements.items():
            blue_points = [
                pos for pos in measurements if pos.is_los and not pos.is_far_from_gpos
            ]
            if blue_points:
    ...
    @final
    def _uwb_find_closest_gpos_by_timestamp(
        self, target_timestamp: float, tag_id: str
    ) -> Optional[Any]:
        ...
        if idx == 0:
            ...
            if abs(before["sensor_timestamp"] - target_timestamp) < abs(
                after["sensor_timestamp"] - target_timestamp
            ):
                closest_data = before
            else:
                closest_data = after
    ...
    @final
    def _uwb_get_tag_pose(
        self, tag_id: str
    ) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """指定されたタグの最新の位置と姿勢を取得（互換性のため残す）"""
        return self._uwb_get_tag_pose_latest(tag_id)
    ...
    @final
    def _uwb_create_blue_only_trajectories(self) -> Dict[str, List[Position]]:
    ...
    @final
    def _uwb_estimate_position_from_blue_points(
        self, tag_id: str
    ) -> Optional[Position]:
